-- Move weekly rollups to SQL view and add calc metadata columns.

ALTER TABLE activities_calc ADD COLUMN activity_type TEXT;
ALTER TABLE activities_calc ADD COLUMN flat_time REAL;
ALTER TABLE activities_calc ADD COLUMN flat_dist REAL;

DROP TABLE IF EXISTS metrics_weekly;
...
days(week, day) AS (
  SELECT week, date(week) FROM weeks
  UNION ALL
  SELECT week, date(day, '+1 day') FROM days WHERE day < date(week, '+6 day')
),
daily AS (
  SELECT
    d.week,
  ...
  FROM days d
  LEFT JOIN base b ON date(b.start_time) = d.day AND b.week = d.week
  GROUP BY d.week, d.day
),
daily_stats AS (
  SELECT
    week,
