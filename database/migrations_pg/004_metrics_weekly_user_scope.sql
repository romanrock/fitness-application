-- Add user scoping to the Postgres metrics_weekly view.
-- Keeps parity with SQLite migration 008_user_scope.sql.
...
daily AS (
  ...
  LEFT JOIN base b ON date(b.start_time) = gs.day::date AND b.week = w.week AND b.user_id = w.user_id
  GROUP BY w.week, w.user_id, gs.day
),
daily_stats AS (
  SELECT
    week,
